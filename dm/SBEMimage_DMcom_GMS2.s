...
//   GMS 2. Another version is available for GMS 3.
// ========================================================================
//
...
// script "tackle_overflow_error.s" in "\sbem-image\misc".
...
void wait_for_command()
{
    ...
    if (DoesFileExist(trigger_file))
    {
		...
		if (command == "MicrotomeStage_GetPositionX") {
			get_value = MicrotomeStage_GetPositionX()
			createok = 0
			try {
				file = CreateFileForWriting(return_file)
				createok = 1
		...
		if (command == "MicrotomeStage_GetPositionY") {
		    get_value = MicrotomeStage_GetPositionY()
			createok = 0
			try {
				file = CreateFileForWriting(return_file)
				createok = 1
		...
		if (command == "MicrotomeStage_GetPositionXY") {
		    get_valueX = MicrotomeStage_GetPositionX()
		    get_valueY = MicrotomeStage_GetPositionY()
			createok = 0
			try {
				file = CreateFileForWriting(return_file)
				createok = 1
		...
		if (command == "MicrotomeStage_GetPositionZ") {
		    get_value = MicrotomeStage_GetPositionZ()
			createok = 0
			try {
				file = CreateFileForWriting(return_file)
				createok = 1
		...
		if (command == "MicrotomeStage_SetPositionX\n") {
			...
			success = MicrotomeStage_SetPositionX(set_value1, 0)
			result(DateStamp() + ": Done.\n")
		}
		// ================================================================
		if (command == "MicrotomeStage_SetPositionY\n") {
		    result(DateStamp() + ": set_value: " + set_value1)
			success = MicrotomeStage_SetPositionY(set_value1, 0)
			result(DateStamp() + ": Done.\n")
		}
		// ================================================================
		if (command == "MicrotomeStage_SetPositionXY\n") {
			...
			success = MicrotomeStage_SetPositionX(set_value1, 0)
			success = MicrotomeStage_SetPositionY(set_value2, 0)
			result(DateStamp() + ": Done.\n")
		}
		// ================================================================
		if (command == "MicrotomeStage_SetPositionXY_Confirm\n") {
			...
			currentX = MicrotomeStage_GetPositionX()
			currentY = MicrotomeStage_GetPositionY()
		...
		if (command == "MicrotomeStage_SetPositionZ\n") {
		    result(DateStamp() + ": set_value: " + set_value1 + "\n")
			success = MicrotomeStage_SetPositionZ(set_value1, 0)
			result(DateStamp() + ": Done.\n")
		}
		// ================================================================
		if (command == "MicrotomeStage_SetPositionZ_Confirm\n") {
		    ...
		    result(DateStamp() + ": set_value: " + set_value1 + "\n")
			// Move:
			success = MicrotomeStage_SetPositionZ(set_value1, 1)
			sleep(0.1)
			// Get new Z position:
			currentZ = MicrotomeStage_GetPositionZ()
			// Check if the current position does NOT match the target 
			...
			if (abs(currentZ - set_value1) > 0.005) {
				...
				result(DateStamp() + ": (target: " + set_value1 + ", actual: " + currentZ + ")\n")
			}
			else {	
				// Create acknowledge file:
				file = CreateFileForWriting(acknowledge_file)
...
remote_control = 1 
// Default settings (slow) for motor speeds, can be updated from SBEMimage:
motor_speed_x = 40
motor_speed_y = 40
