...
class OnTheFlyOverviewN5Converter:
    ...
    def __init__(self, acq, ovm, ov_index):
        ...
        self._conversion_metadata = ConversionMetadata()
        self._conversion_metadata.create_metadata(-1, [], 0,
                                                  ovm[ov_index].centre_sx_sy,
                                                  ovm[ov_index].rotation,
                                                  ovm[ov_index].frame_size,
    ...
    def update_ov_settings(self):
        ...
        if overview.centre_sx_sy != self._conversion_metadata.ov_centre_sx_sy:
            self._conversion_metadata.ov_centre_sx_sy = overview.centre_sx_sy
            settings_changed = True
        if overview.rotation != self._conversion_metadata.ov_rotation:
            self._conversion_metadata.ov_rotation = overview.rotation
            settings_changed = True
        if overview.frame_size != self._conversion_metadata.ov_size:
            self._conversion_metadata.ov_size = overview.frame_size
            settings_changed = True
        if overview.pixel_size != self._conversion_metadata.ov_pixel_size:
            self._conversion_metadata.ov_pixel_size = overview.pixel_size
            settings_changed = True
        if self.acq.slice_thickness != self._conversion_metadata.slice_thickness:
            self._conversion_metadata.slice_thickness = self.acq.slice_thickness
            settings_changed = True
    ...
    def _initialise_new_n5(self):
        ...
        if not os.path.isfile(n5_path):
            ...
            # TODO - try to read configuration file, and figure out what to do
            # read json, set current parameters to that. If same, carry on, if not, increment the index

        self._current_dataset = BdvDataset(n5_path, setup_id=0, timepoint=0)

        # TODO - save settings to cfg


    def _create_conversion_metadata(self):
        # current settings: these are settings that if changed, need a new n5 to be created
        # TODO - others to go here? e.g. if current slice counter or z_diff are manually changed?
        # TODO - current images are placed at (0,0) for xy, in future it would be good to place them at their proper
        # location via the affine transform. Then people could move their overviews and still have them display
        ...
        metadata = {"current_id": -1,
                    "current_dataset_shape": current_dataset_shape,
                    "first_slice_no_of_current_dataset": 0,
                    "converted_files": []}
        file_settings_dict = {"ov_centre_sx_sy": self.ovm[self.ov_index].centre_sx_sy,
                              "ov_rotation": self.ovm[self.ov_index].rotation,
                              "ov_size": ov_size,
                              "ov_pixel_size": self.ovm[self.ov_index].pixel_size,
                              "slice_thickness": self.acq.slice_thickness}
        metadata["converted_files"].append(file_settings_dict)

        return metadata

    def _save_conversion_metadata(self):
        json_file = os.path.join(self._output_dir, self.JSON_FILENAME)
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)

    def _read_conversion_metadata(self):
        json_file = os.path.join(self._output_dir, self.JSON_FILENAME)
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
